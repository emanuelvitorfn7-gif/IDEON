...
def carregar(caminho):
    ...
    migrar(db)
    if db.execute("SELECT id FROM usuarios WHERE email = ?",
                  ("demo.prof@exemplo.com",)).fetchone():
        db.close()
        return {"status": "existente", "caminho": caminho}

    prof = db.execute(
        "INSERT INTO usuarios (nome, email, senha_hash, perfil) VALUES (?, ?, ?, ?)",
        ("Prof. Demo (fictício)", "demo.prof@exemplo.com",
         generate_password_hash(SENHA), "professor")).lastrowid
    alunos = {}
    for nome, email in (("Ana Demo (fictícia)", "demo.ana@exemplo.com"),
        ...
        alunos[email] = db.execute(
            ...
            (nome, email, generate_password_hash(SENHA))).lastrowid
    turma = db.execute(
        "INSERT INTO turmas (nome, descricao, codigo, professor_id)"
        " VALUES (?, ?, ?, ?)",
        ("Ciências 101 (demonstração)", "Turma fictícia para demonstração.",
         "DEMO01", prof)).lastrowid
    for email, uid in alunos.items():
        db.execute("INSERT INTO inscricoes (turma_id, aluno_id) VALUES (?, ?)",
                   (turma, uid))
    mat = db.execute(
        "INSERT INTO materiais (turma_id, professor_id, titulo, origem, estado)"
        " VALUES (?, ?, ?, 'texto', 'liberado')",
        (turma, prof, ia.MATERIAL_DEMO["titulo"])).lastrowid
    db.executemany(
    ...
    atv = db.execute(
        ...
        " VALUES (?, ?, ?, 'publicada', 1)",
        (turma, mat, "Atividade demonstrativa (fictícia)")).lastrowid
    qids = []
    for q in validas:
        qids.append(db.execute(
            ...
            " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)",
            (atv, q["enunciado"], q["alternativas"][q["correta"]],
             json.dumps(q["alternativas"], ensure_ascii=False), q["correta"],
             q["explicacao"], q["assunto"], q["pagina"], q["evidencia"])).lastrowid)
    db.commit()

    def tentativa(uid, respostas):
        tid = db.execute(
            "INSERT INTO tentativas (atividade_id, aluno_id, total) VALUES (?, ?, ?)",
            (atv, uid, len(qids))).lastrowid
        for qid, alt in zip(qids, respostas):
            correta = db.execute(
                "SELECT correta FROM questoes WHERE id = ?", (qid,)).fetchone()["correta"]
            db.execute("INSERT INTO tentativa_respostas (tentativa_id, questao_id,"
    ...
    aberta = db.execute(
        "INSERT INTO tentativas (atividade_id, aluno_id, total) VALUES (?, ?, ?)",
        (atv, alunos["demo.carla@exemplo.com"], len(qids))).lastrowid
    for qid, alt in zip(qids[:2], [0, 0]):
        db.execute("INSERT INTO tentativa_respostas (tentativa_id, questao_id,"
                   " alternativa, correta) VALUES (?, ?, ?, 1)", (aberta, qid, alt))
    db.commit()
    db.close()
