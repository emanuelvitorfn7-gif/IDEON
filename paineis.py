...
def concluidas(db, aluno_id, turma_id=None):
    q = ("SELECT t.* FROM tentativas t JOIN atividades a ON a.id = t.atividade_id"
...
def resumo_aluno(db, aluno_id):
    ...
    historico = db.execute(
        ...
        " WHERE t.aluno_id = ? AND t.concluida_em IS NOT NULL"
        " ORDER BY t.concluida_em DESC", (aluno_id,)).fetchall()
    evolucao = []
    for aid, tents in ativs.items():
        ...
        titulo = db.execute(
            "SELECT titulo FROM atividades WHERE id = ?", (aid,)).fetchone()["titulo"]
    ...
    for aid, tents in ativs.items():
        ...
        erros = db.execute(
            ...
            " GROUP BY q.assunto, q.pagina ORDER BY n DESC", (ultima,)).fetchall()
        titulo = db.execute(
            "SELECT titulo FROM atividades WHERE id = ?", (aid,)).fetchone()["titulo"]
        for e in erros:
            recomend.append({"atividade": titulo, "assunto": e["assunto"],
                             "pagina": e["pagina"], "erros": e["n"],
                             "material_id": e["material_id"]
                             if e["mat_estado"] == "liberado" else None})
    total = xp.total_xp(db, aluno_id)
    return {"xp_total": total, "nivel": xp.nivel_info(total),
            "dias": xp.dias_ativos_total(db, aluno_id),
            "concluidas": len(ativs), "historico": historico,
            "evolucao": evolucao, "recomendacoes": recomend}
...
def ranking_turma(db, turma_id):
    ...
    alunos = db.execute(
        ...
        " WHERE i.turma_id = ? ORDER BY u.nome", (turma_id,)).fetchall()
    linhas = []
    for a in alunos:
        xp_turma = db.execute(
        ...
        for at in db.execute(
            ...
            if len(tents) >= 2:
                diffs.append(_pct(tents[-1]["acertos"], tents[-1]["total"])
                             - _pct(tents[0]["acertos"], tents[0]["total"]))
        linhas.append({"nome": a["nome"], "xp": xp_turma, "dias": dias,
...
def stats_turma(db, turma_id):
    ...
    for at in db.execute(
            ...
            " ORDER BY id", (turma_id,)):
        concluintes, somas = 0, []
        for al in inscritos:
            tents = [t for t in concluidas(db, al["id"], turma_id)
            ...
            for r in db.execute(
                    ...
                    " JOIN questoes q ON q.id = r.questao_id"
                    " WHERE r.tentativa_id = ?", (tents[0]["id"],)):
                ag = assuntos.setdefault(r["assunto"] or "Geral", [0, 0])
                ag[1] += 1
                ag[0] += r["correta"]
        por_atividade.append({
            "titulo": at["titulo"], "concluintes": concluintes,
    ...
    for al in inscritos:
        ...
        if not meds:
            ...
            abaixo.append({"nome": al["nome"],
                           "media": round(sum(meds) / len(meds) * 100)})
    return {"n_inscritos": len(inscritos), "por_atividade": por_atividade,
            "assuntos": [{"assunto": k, "pct": round(v[0] / v[1] * 100), "n": v[1]}
                         for k, v in sorted(assuntos.items())],
            "sem_conclusao": sem_conclusao, "abaixo": abaixo}
